# 28. Docker 백업과 복구 (Backup & Recovery)

> **학습 목표**: Docker 환경에서 데이터 손실을 방지하기 위한 체계적인 백업 전략을 수립하고, 재해 상황에서 신속하게 시스템을 복구할 수 있는 능력을 갖춥니다.
...
<details>
<summary><strong>Q1: 볼륨 백업과 데이터베이스 백업의 차이점은 무엇인가요?</strong></summary>

**A**: 볼륨 백업은 파일 시스템 레벨, 데이터베이스 백업은 논리적 레벨에서 데이터를 백업합니다.

**상세 설명**:
- **볼륨 백업**: 파일 그대로 복사, 빠르지만 일관성 보장 어려움
- **데이터베이스 백업**: pg_dump 등 사용, 느리지만 일관성 보장
- **권장**: 두 가지 모두 수행 (이중 안전장치)
- **예시**: PostgreSQL의 경우 pg_dump + 볼륨 백업 병행

</details>

<details>
<summary><strong>Q2: 3-2-1 백업 규칙이 무엇인가요?</strong></summary>

**A**: 데이터의 3개 복사본을 2가지 다른 매체에, 1개는 오프사이트에 보관하는 원칙입니다.

**구체적 적용**:
- **3개 복사본**: 원본 + 로컬 백업 + 원격 백업
- **2가지 매체**: SSD/HDD + 클라우드 스토리지
- **1개 오프사이트**: AWS S3, Azure Blob 등 다른 지역에 보관
- **이유**: 하드웨어 고장, 자연재해, 랜섬웨어 등 모든 위협 대비

</details>

<details>
<summary><strong>Q3: RPO와 RTO의 차이점을 설명해주세요.</strong></summary>

**A**: RPO는 허용 가능한 데이터 손실 시간, RTO는 허용 가능한 서비스 중단 시간입니다.

**비교**:
- **RPO (Recovery Point Objective)**: 최대 몇 시간치 데이터 손실 허용?
  - 예: RPO 1시간 → 1시간마다 백업 필요
- **RTO (Recovery Time Objective)**: 얼마나 빨리 복구해야 하는가?
  - 예: RTO 30분 → 30분 내에 서비스 재개
- **관계**: RPO 짧을수록 백업 빈도 증가, RTO 짧을수록 복구 절차 간소화 필요
...
**Q1: Docker 볼륨을 백업하는 기본 명령어를 작성하고 설명하세요.**

**A**: `docker run`으로 임시 컨테이너를 만들어 볼륨을 tar 파일로 압축합니다.
...
docker run --rm \
  ...
  -v $(pwd)/backups:/backup \
  alpine \
  tar czf /backup/my-volume-$(date +%Y%m%d).tar.gz -C /data .
```

**설명**:
- `--rm`: 작업 완료 후 컨테이너 자동 삭제
- `-v my-volume:/data`: 백업할 볼륨 마운트
- `-v $(pwd)/backups:/backup`: 백업 파일 저장 위치
- `tar czf`: gzip 압축하여 tar 파일 생성
- `$(date +%Y%m%d)`: 날짜를 파일명에 포함

---

**Q2: PostgreSQL 데이터베이스를 백업하고 복구하는 명령어를 작성하세요.**

**A**: `pg_dump`로 백업하고 `psql`로 복구합니다.
...
**복구**:
```bash
gunzip -c backup.sql.gz | docker exec -i postgres-container psql -U username dbname
```

**주의사항**:
- 백업 전 컨테이너 중지 불필요 (온라인 백업 가능)
- 복구 시에는 관련 서비스 중지 권장
- 큰 데이터베이스는 `pg_dump -Fc` (커스텀 포맷) 사용

---

### 중급 레벨

**Q3: 자동화된 백업 시스템을 구축하기 위한 전체 전략을 설명하세요.**

**A**: Cron 스케줄링, 로테이션 정책, 원격 저장소 동기화를 포함한 종합 전략이 필요합니다.

**전체 구조**:

**1. 백업 스크립트** (`backup-all.sh`):
```bash
#!/bin/bash
DATE=$(date +%Y%m%d-%H%M%S)
...
# 1. 데이터베이스 백업
docker exec postgres pg_dumpall -U admin | gzip > $BACKUP_DIR/db.sql.gz

# 2. 볼륨 백업
docker run --rm -v my-volume:/data -v $(pwd)/$BACKUP_DIR:/backup alpine tar czf /backup/volume.tar.gz -C /data .

# 3. 설정 파일 백업
tar czf $BACKUP_DIR/configs.tar.gz docker-compose.yml .env
...
# 2. 백업에서 복구
gunzip -c $BACKUP/db.sql.gz | docker exec -i postgres psql -U admin

# 3. 볼륨 복구
docker run --rm -v my-volume:/data -v $BACKUP:/backup alpine tar xzf /backup/volume.tar.gz -C /data
...
**핵심 포인트**:
- **자동화**: Cron으로 백업 자동 실행
- **로테이션**: 오래된 백업 자동 삭제로 디스크 관리
- **오프사이트**: S3 등 원격 저장소에 백업 보관
- **테스트**: 정기적으로 복구 테스트 수행 필수
