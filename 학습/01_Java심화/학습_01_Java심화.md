...
### Effective Java 학습 방침

**교재**: Effective Java 3rd Edition (보유)
**방침**: 책 전체를 읽되, 정리 깊이를 두 단계로 나눈다.

| 구분 | 대상 | 학습 방식 |
|------|------|----------|
| **깊게 정리** | Step 1~8 + Step 14에 선별된 41개 아이템 | 학습 문서 작성 + 코드 실습 + 면접 답변 준비 |
| **1회독** | 나머지 49개 아이템 (각 Step 하단 + 9장/12장) | 책을 읽고 이해만. 별도 정리 안 함 |

> 면접/실무에서 자주 나오는 핵심 아이템은 이미 Step에 포함되어 있다.
> 60시간 안에 동시성·JVM·자료구조까지 커버해야 하므로, Effective Java에만 시간을 과투자하지 않는다.
...
### Effective Java 1회독 — 일반적인 프로그래밍 원칙 (9장)

> Step에 포함되지 않은 챕터. 전체 1회독 대상.
...
### Effective Java 1회독 — 직렬화 (12장)

> Step에 포함되지 않은 챕터. 전체 1회독 대상. (Step 19 I/O에서 직렬화 개념은 별도 학습)

- Item 85: 자바 직렬화의 대안을 찾으라
- Item 86: Serializable을 구현할지는 신중히 결정하라
- Item 87: 커스텀 직렬화 형태를 고려해 보라
- Item 88: readObject 메서드는 방어적으로 작성하라
- Item 89: 인스턴스 수를 통제해야 한다면 readResolve보다는 열거 타입을 사용하라
- Item 90: 직렬화된 인스턴스 대신 직렬화 프록시 패턴을 검토하라
...
| 자료 | 용도 |
|------|------|
| Effective Java 3rd Edition (Joshua Bloch) | **보유**. Step 1~8 깊게 정리 + 나머지 1회독 |
| 김영한 자바 고급 1 (인프런) | Step 10~14 보조. 동시성 강의 참고 시 |
