...
<details>
<summary><strong>Q1: root 사용자로 컨테이너를 실행하면 안 되는 이유가 뭔가요?</strong></summary>

**A**: 컨테이너가 침해될 경우 공격자가 root 권한을 얻어 호스트 시스템까지 장악할 수 있기 때문입니다.

**상세 설명**:
- root로 실행 시: 컨테이너 탈출 → 호스트 root 권한 획득 가능
- non-root 실행 시: 컨테이너 내부 권한만 획득, 피해 최소화
- Best Practice: Dockerfile에서 USER 지시어로 non-root 사용자 지정
- 예시: `RUN adduser -D appuser && USER appuser`

</details>

<details>
<summary><strong>Q2: Trivy로 이미지 스캔 시 발견된 취약점을 어떻게 해결하나요?</strong></summary>

**A**: 베이스 이미지 업데이트, 패키지 업그레이드, 또는 대안 이미지 사용으로 해결합니다.

**해결 방법**:
1. 베이스 이미지 버전 업그레이드: `FROM node:18` → `FROM node:20`
2. 시스템 패키지 업데이트: `RUN apk upgrade --no-cache`
3. 취약한 의존성 업그레이드: `package.json` 버전 업데이트
4. Distroless 이미지 사용: 불필요한 패키지 제거

</details>

<details>
<summary><strong>Q3: Docker Secrets와 환경 변수의 차이점은 무엇인가요?</strong></summary>

**A**: Docker Secrets는 암호화되어 메모리에만 존재하며, 환경 변수보다 훨씬 안전합니다.

**비교**:
- **환경 변수**: `docker inspect`로 누구나 확인 가능, 평문 노출
- **Docker Secrets**: 암호화 전송, 메모리에만 마운트, `docker inspect`로 확인 불가
- **권장**: 비밀번호, API 키는 반드시 Secrets 사용
